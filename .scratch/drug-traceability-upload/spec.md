...
需要一个轻量级 Web 管理系统，实现定时自动上传 + 手动上传 + 结果查询的完整闭环。
...
### 自动化上传
21. 作为系统，我想每天 20:00 自动执行 SQL 查询从 ERP 获取当天单据并上传
22. 作为系统，我想在 API 无响应时自动重试最多 3 次（间隔 30 秒），网络超时重试但业务错误不重试
23. 作为系统，我想 API 调用间隔保持 0.33 秒，避免触发平台限流
24. 作为系统，我想使用文件锁防止 cron 任务并发执行

### 追溯码处理
25. 作为系统，我想将同一单号的追溯码用英文逗号拼接为一个字符串
26. 作为系统，当一个单号的追溯码超过 3500 个时，自动拆分为 单号_1、单号_2、单号_3... 分批上传

### 日志
27. 作为系统，我想将每次 API 调用的请求快照和响应结果写入 JSONL 文件永久保存
28. 作为系统，我想将每次 API 调用的结果实时同步到 SQLite 便于 Web 查询
29. 作为系统，我想定时清理 SQLite 中超过 3 个月的记录，控制数据库体积

### 导航与UI
30. 作为用户，我想要左侧菜单栏默认收起（仅显示 SVG 图标），点击展开后显示图标+文字
31. 作为用户，我想要响应式布局，在不同屏幕尺寸下都能正常使用
...
- 网络超时：curl 超时、连接失败 → 抛 NetworkException
- 业务错误：API 返回错误码 → 抛 BusinessException
- 正常响应 → 返回解析后的数组

**LogWriter** — 日志写入
- JSONL：追加写入 `logs/api_YYYY-MM-DD.jsonl`，每行一条 JSON
- SQLite：INSERT 到 `upload_logs` 表
- 清理：Cron 每月 1 日执行，DELETE 3 个月前的记录
...
CREATE TABLE upload_tasks (
    ...
    source TEXT NOT NULL DEFAULT 'cron',     -- cron/manual/import
